...
module.exports = {
	...
	receivedPostback: function(event) {
		...
		if(payload.hasOwnProperty("gettingStarted")){
			...
			database.addToWatchedList(payload.userId, payload.willWatch, payload.showType);
			return messageBuilder.getWillWatchResponse(senderID);
		}else if(payload.hasOwnProperty("startOver")){
			//later we should send back a better message
			return messageBuilder.getGettingStartedResponse(senderID);
		}
