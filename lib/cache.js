...
const myCache = new NodeCache({
	stdTTL: cachedItemsTtl, checkperiod: 10
});
...
function getCachedSeenShows(recipientId, showType, genre) {
	const key = createShowKey("aboutToGetShowsFromDB", recipientId, showType, genre);
	insert(key, true, 0);
...
myCache.on("expired", function(key, value) {
	...
	if (showKeyObj.showSeen) {
		...
		//console.log("about to add seenList to DB...");
...
function deleteKey(key) {
	...
	if (value == 1)
		...
		console.log("when deleting got val= " + value);
}
