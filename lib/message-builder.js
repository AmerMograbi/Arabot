		...
		database.findByGenre(chosenShowType, chosenGenre)
			.then(showsFound => {
				...
				const elements = showsFound.map(s => {
					...
					const buttons = getShowButtons(s.description, s._id, recipientId, chosenShowType);
					return buildElement(title, s.description, youtubeTrailerUrl, imageUrl, buttons);
				});
...
function getShowButtons(showDescription, showDbId, userId, showType) {
	const buttonData = arabicText[showButtons];
	let buttons = [];
	...
	const willWatchPayload = JSON.stringify(buildWillWatchPayload(showDbId, userId, showType));
...
function buildWillWatchPayload(showDbId, userId, showType) {
	return {
		willWatch: showDbId,
		userId: userId,
		showType: showType
