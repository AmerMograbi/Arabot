...
module.exports = {
			...
			case "moreInfoResponse":
				{
					...
					if(chosenResponse.hasOwnProperty("seen")){
						const showDbId = chosenResponse.seen;
						return respondToseen(recipientId, showDbId, showType, genre);
	...
	getseenResponse: function(recipientId, showDbId, showType, genre) {
		return respondToseen(recipientId, showDbId, showType, genre);
	},
...
function respondToNextShow(recipientId, showType, genre, currShowId){
	...
	database.addToSeenList(recipientId, currShowId, showType, genre);
...
function respondToseen(recipientId, showDbId, showType, genre){
	const payload = {
		state: buildState("seenResponse")
	};
	database.addToSeenList(recipientId, showDbId, showType, genre);
	return buildQuickReplyMessage(recipientId, payload);
}
...
function buildMoreInfoContent(description, showDbId, showType, genre)
{
	...
	const seenPayload = buildseenPayload(showDbId, showType, genre);
	...
	moreInfoResponse[seenPayload] = "seen";
...
function buildButtonTemplateMessage(recipientId, description) {
	...
	buttons.push(buildPostBackButton("postback", buttonData.seen, "seen"));
...
function getShowButtons(showDescription, showDbId, showType, genre) {
	...
	buttons.push(buildPostBackButton("postback", buttonData.seen, seenPayload));
...
function buildseenPayload(showDbId, showType, genre) {
	return JSON.stringify({
		seen: showDbId,
		showType: showType,
		genre: genre
