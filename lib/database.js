	...
	findNotWatchedShows: function(userId, showType, genres) {
		...
		return new Promise((resolve, reject) => {
			getWatchedShows(userId, showType, genres)
				.then(watched => {
					const watchedObjIds = watched.map(w => new ObjectId(w));
					shows.find({
						"genres": genres,
						"_id": {
							$nin: watchedObjIds
	...
	return new Promise((resolve, reject) => {
		...
		users.findOne({
			...
			if (err) reject(err);
				...
				if (!doc || !doc.hasOwnProperty(watched) ||
				 !doc[watched].hasOwnProperty("genres") ||
				 !doc[watched].genres.hasOwnProperty(genre)) {
					resolve([]);
				} else {
